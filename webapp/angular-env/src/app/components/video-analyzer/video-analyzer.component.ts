...
@Component({
  ...
  styleUrl: './video-analyzer.component.css'
})
export class VideoAnalyzerComponent implements OnChanges, OnDestroy {
  ...
  private readonly LANDMARKS_STORAGE_KEY = 'face.landmarks.enabled';

  landmarksEnabled: boolean =
    (typeof window === 'undefined') ? true : window.localStorage?.getItem('face.landmarks.enabled') !== '0';
  ...
  setShowGrid(v: boolean): void {
    this.showGrid = v;
    try { window.localStorage?.setItem(this.GRID_STORAGE_KEY, v ? '1' : '0'); } catch {}
  }
  ...
  setLandmarksEnabled(v: boolean): void {
    this.landmarksEnabled = v;
    try { window.localStorage?.setItem(this.LANDMARKS_STORAGE_KEY, v ? '1' : '0'); } catch {}
  }

  toggleLandmarks(): void {
    this.setLandmarksEnabled(!this.landmarksEnabled);
  }

  @HostListener('window:keydown', ['$event'])
  onKey(ev: KeyboardEvent): void {
    const k = ev.key.toLowerCase();
    if (k === 'g') this.toggleGrid();
    if (k === 'l') this.toggleLandmarks();
  }
  ...
  private async ensureDefaultCameraIfNeeded(): Promise<void> {
    ...
    try {
      ...
      console.warn('[VideoAnalyzer] getUserMedia failed:', e);
    }
  }
